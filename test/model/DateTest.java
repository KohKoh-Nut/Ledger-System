...
class DateTest {
    ...
    @Test
    @DisplayName("Current date should always be valid")
    void current_returnsValidDate() {
        Date now = Date.current();
        assertTrue(now.isValid(), "The date generated by current() should be valid");

        // Verify it matches system clock
        LocalDate systemNow = LocalDate.now();
        String expected = systemNow.getDayOfMonth() + "/" +
                systemNow.getMonthValue() + "/" +
                systemNow.getYear();
        assertEquals(expected, now.toString());
    }
}